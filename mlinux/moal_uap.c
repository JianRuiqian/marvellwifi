...
woal_uap_snmp_mib(struct net_device *dev, struct ifreq *req)
{
	...
	if (param.action) {
		if (copy_from_user(value, (char *)req->ifr_data + sizeof(param),
	...
	if (!param.action) {	/* GET */
		if (copy_to_user
		    ((char *)req->ifr_data + sizeof(param), &snmp->param.oid_value,
		     MIN(param.oid_val_len, sizeof(t_u32)))) {
			PRINTM(MERROR, "Copy from user failed\n");
			ret = -EFAULT;
			goto done;
...
woal_uap_domain_info(struct net_device *dev, struct ifreq *req)
{
	...
	if (param.action) {
		/* get tlv header */
		if (copy_from_user
		    (tlv, (char *)req->ifr_data + sizeof(param), TLV_HEADER_LEN)) {
			PRINTM(MERROR, "Copy from user failed\n");
			ret = -EFAULT;
			goto done;
		}
		...
		/* get full tlv */
		if (copy_from_user(tlv, (char *)req->ifr_data + sizeof(param),
	...
	if (!param.action) {	/* GET */
		...
		if (copy_to_user
		    ((char *)req->ifr_data + sizeof(param), &cfg11d->param.domain_tlv,
		     TLV_HEADER_LEN + tlv_data_len)) {
			PRINTM(MERROR, "Copy from user failed\n");
			ret = -EFAULT;
			goto done;
...
woal_uap_tx_bf_cfg(struct net_device *dev, struct ifreq *req)
{
	...
	if (!param.action)
		...
		action = MLAN_ACT_SET;
	if (copy_from_user(&bf_cfg, (char *)req->ifr_data + sizeof(tx_bf_cfg_para_hdr),
			   sizeof(bf_cfg))) {
		PRINTM(MERROR, "Copy from user failed\n");
		ret = -EFAULT;
		goto done;
	...
	/* Copy to user */
	if (copy_to_user((char *)req->ifr_data + sizeof(tx_bf_cfg_para_hdr),
...
woal_uap_bss_cfg_ioctl(struct net_device *dev, struct ifreq *req)
{
	...
	/* Get action */
	if (copy_from_user(&action, (char *)req->ifr_data + offset, sizeof(action))) {
		PRINTM(MERROR, "Copy from user failed\n");
		ret = -EFAULT;
		goto done;
	}
	...
	if (ioctl_req->action == MLAN_ACT_SET) {
		...
		if (copy_from_user
		    (&bss->param.bss_config, (char *)req->ifr_data + offset,
	...
	if (ioctl_req->action == MLAN_ACT_GET) {
		...
		if (copy_to_user
		    ((char *)req->ifr_data + offset, &bss->param.bss_config,
		     sizeof(mlan_uap_bss_param))) {
			PRINTM(MERROR, "Copy to user failed!\n");
			ret = -EFAULT;
			goto done;
...
woal_uap_ap_cfg_parse_data(mlan_uap_bss_param * ap_cfg, char *buf)
{
	...
	while (begin) {
		...
		if (!opt || !value || !value[0]) {
			...
			if (!strncmp(value, "open", strlen("open"))) {
