...
static const region_code_mapping_t hw_region_code_mapping[] = {
	{"US ", 0x10},		/* US FCC */
	{"CA ", 0x20},		/* IC Canada */
	{"KR ", 0x30},		/* Korea */
	{"CN ", 0x50},		/* China */
...
region_code_2_string(t_u8 region_code)
{
	...
	for (i = 0; i < ARRAY_SIZE(hw_region_code_mapping); i++) {
		if (hw_region_code_mapping[i].code == region_code) {
			LEAVE();
			return (char *)hw_region_code_mapping[i].region;
		}
	}
	LEAVE();
	return NULL;
...
#endif
	ENTER();
	for (i = 0; i < handle->priv_num && ((priv = handle->priv[i]) != NULL); i++) {
#ifdef STA_SUPPORT
...
woal_hostcmd_ioctl(struct net_device *dev, struct ifreq *req)
{
	...
	/* get command header */
	if (copy_from_user
	    (&cmd_header, (char *)req->ifr_data + sizeof(buf_len),
	...
	/* get the whole command from user */
	if (copy_from_user
	    (misc->param.hostcmd.cmd, (char *)req->ifr_data + sizeof(buf_len),
	     misc->param.hostcmd.len)) {
		PRINTM(MERROR, "copy from user failed\n");
		ret = -EFAULT;
		goto done;
	...
	if (copy_to_user
	    ((char *)req->ifr_data + sizeof(buf_len), (t_u8 *) misc->param.hostcmd.cmd,
	     misc->param.hostcmd.len)) {
		ret = -EFAULT;
		goto done;
	}
...
#define MV_ETH_FRAME_LEN      1514
	...
	if (copy_from_user
	    (pmbuf->pbuf + pmbuf->data_offset,
	     (char *)req->ifr_data + sizeof(packet_len),
...
woal_get_version(moal_handle * handle, char *version, int max_len)
{
	...
	snprintf(version, max_len, (const char *)handle->driver_version, fw_ver);
...
{
	char *ptr = buf + strlen("BGSCAN-CONFIG") + 1;
	...
	while (buf_left >= 2) {
		...
		case WEXT_CSCAN_SSID_SECTION:
			...
			if (ssid_len &&
				...
				strncpy((char *)priv->scan_cfg.ssid_list[num_ssid].ssid,
					ptr + 2, ssid_len);
...
woal_set_combo_scan(moal_private * priv, char *buf, int length)
{
	...
	char *ptr = buf + WEXT_CSCAN_HEADER_SIZE;
	...
	while (buf_left >= 2) {
		...
		case WEXT_CSCAN_SSID_SECTION:
			...
			if (ssid_len &&
			    (num_ssid < (MRVDRV_MAX_SSID_LIST_LENGTH - 1))) {
				strncpy((char *)scan_cfg.ssid_list[num_ssid].ssid,
					ptr + 2, ssid_len);
...
{
	mlan_status ret = MLAN_STATUS_SUCCESS;
	char *ptr = buf + NL80211_SCANCFG_HEADER_SIZE;
