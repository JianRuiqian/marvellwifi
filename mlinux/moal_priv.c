...
woal_tx_bf_cfg_ioctl(moal_private * priv, struct iwreq *wrq)
{
	...
	if (action == MLAN_ACT_GET) {
		...
		case BF_GLOBAL_CONFIGURATION:
			data_length +=
				sprintf((char *)buf + data_length, "%d ",
			...
			break;
		case SET_GET_BF_PERIODICITY:
			data_length += sprintf((char *)buf + data_length,
			...
			data_length += sprintf((char *)buf + data_length, "%c", ' ');
			data_length +=
				sprintf((char *)buf + data_length, "%d",
		...
		case TX_BF_FOR_PEER_ENBL:
			for (i = 0; i < bf_cfg.no_of_peers; i++) {
				...
				data_length +=
					sprintf((char *)buf + data_length, "%c", ' ');
				data_length +=
					sprintf((char *)buf + data_length, "%d;",
						tx_bf_peer->bf_enbl);
				data_length +=
			      ...
			      mlan_scan_resp * scan_resp, t_u32 scan_start)
{
	...
	pbuffer_end = (t_u8 *)wrq->u.data.pointer + wrq->u.data.length - 1;
