...
/* Port header files */
#include <ctype.h>
#include "../mport/list.h"
#include "../mport/atomic.h"
...
extern int request_firmware(const struct firmware **fw, const char *file);
extern void release_firmware(const struct firmware *fw);
extern void get_firmware_data(const struct firmware *fw, u8 *buf, u32 len);
...
#include        "../mport/wireless.h"
#include        "../mport/iw_handler.h"
#include        "moal_wext.h"
#endif
#ifdef STA_WEXT
#include        "moal_priv.h"
...
//			  jiffies + ((timer->time_period * HZ) / 1000));
        rt_tick_t tick;
        tick = (timer->time_period * HZ) / 1000;
        rt_timer_control(&timer->tl, RT_TIMER_CTRL_SET_TIME, (void *)&tick);
...
//	timer->tl.data = (t_ptr)timer;
    rt_timer_init(&timer->tl, "mw_timer", woal_timer_handler, 
                    (void *)timer, 0, RT_TIMER_FLAG_ONE_SHOT);
...
woal_mod_timer(pmoal_drv_timer timer, t_u32 millisecondperiod)
{
    ...
    rt_tick_t tick;
    tick = (millisecondperiod * HZ) / 1000;
    rt_timer_control(&timer->tl, RT_TIMER_CTRL_SET_TIME, (void *)&tick);
    rt_timer_start(&timer->tl);
...
//	thr->task = kthread_run(threadfunc, thr, "%s", name);    
    thr->task = rt_thread_create(name, (void (*)(void *))threadfunc, thr, 
                                1024, RT_THREAD_PRIORITY_MAX - 2, 10);
    if (thr->task != RT_NULL)
        rt_thread_startup(thr->task);
}
#endif /* REASSOCIATION */
...
#if 1//LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19)
