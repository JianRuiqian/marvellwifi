...
int _woal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id);
void _woal_sdio_remove(struct sdio_func *func);

rt_int32_t woal_sdio_probe(struct rt_mmcsd_card *card)
{
    return _woal_sdio_probe((struct sdio_func *)card->sdio_function[1], 
        (struct sdio_device_id *)&wlan_ids[0]);
}

rt_int32_t woal_sdio_remove(struct rt_mmcsd_card *card)
{
    _woal_sdio_remove((struct sdio_func *)card->sdio_function[1]);
...
#endif
#endif
static struct rt_sdio_driver REFDATA wlan_sdio = {
	.name = "wlan_sdio",
	.id/*_table*/ = (struct sdio_device_id *)&wlan_ids[0],
...
woal_sdio_update_card_type(moal_handle * handle, t_void * card)
{
	...
	/* Update card type */
	if (cardp->func->product == SD_DEVICE_ID_8797)
		handle->card_type = CARD_TYPE_SD8797;
	else if (cardp->func->product == SD_DEVICE_ID_8782)
		handle->card_type = CARD_TYPE_SD8782;
    else if (cardp->func->product == SD_DEVICE_ID_8801)
 ...
 */
int
_woal_sdio_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	PRINTM(MMSG, "vendor=0x%4.04X device=0x%4.04X class=%d function=%d\n",
	       func->manufacturer, func->product, func->func_code, func->num);
 ...
 */
void
_woal_sdio_remove(struct sdio_func *func)
{
	struct sdio_mmc_card *card;
...
woal_bus_register(void)
{
    ...
    for (index = 0; index < ARRAY_SIZE(wlan_ids); index ++)
    {
        wlan_sdio.id = (struct rt_sdio_device_id *)&wlan_ids[index];
...
woal_register_dev(moal_handle * handle)
{
	...
	handle->hotplug_device = NULL/*&func->dev*/;
...
woal_sdio_set_bus_clock(moal_handle * handle, t_u8 option)
{
	...
	if (option == MTRUE) {
		...
		if (cardp->host_clock)
			host->io_cfg.clock = cardp->host_clock;
	} else {
		/* backup value if non-zero, then clear */
		if (host->io_cfg.clock)
			cardp->host_clock = host->io_cfg.clock;
		host->io_cfg.clock = 0;
	}

	host->ops->set_iocfg(host, &host->io_cfg);
	LEAVE();
	return MLAN_STATUS_SUCCESS;
}
