...
//	memcpy(pbuf, handle->firmware->data + offset, len);
    get_firmware_data(handle->firmware, pbuf, len);
...
#ifdef STA_WEXT
		if (IS_STA_WEXT(cfg80211_wext))
			woal_send_iwevcustom_event(priv, (char *)pmevent->event_buf);
#endif
		woal_broadcast_event(priv, pmevent->event_buf,
				     pmevent->event_len);
		break;
...
#endif
		woal_broadcast_event(priv, pmevent->event_buf,
				     strlen((const char *)pmevent->event_buf));
		break;
...
moal_print(IN t_void * pmoal_handle, IN t_u32 level, IN char *pformat, IN ...)
{
	...
	if (level & MHEX_DUMP) {
		...
		if (drvdbg & level) {
            ...
            length = rt_vsnprintf(rt_log_buf, sizeof(rt_log_buf) - 1, pformat, args);
