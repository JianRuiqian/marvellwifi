...
static const t_u8 wpa_oui[CIPHER_SUITE_MAX][4] = {
...
static const t_u8 rsn_oui[CIPHER_SUITE_MAX][4] = {
	...
	if (((pbss_desc->prsn_ie) &&
		...
		ie_body =
			(IEBody *) (((t_u8 *) pbss_desc->prsn_ie->data) +
				    RSN_GTK_OUI_OFFSET);
		oui = (t_u8 *)&rsn_oui[cipher_suite][0];
		   ...
		   t_u32 cipher_suite)
{
	...
	if (((pbss_desc->pwpa_ie) &&
		...
		oui = (t_u8 *)&wpa_oui[cipher_suite][0];
		ret = search_oui_in_ie(pmadapter, ie_body, oui);
		if (ret) {
			LEAVE();
			return ret;
...
wlan_scan_process_results(IN mlan_private * pmpriv)
{
    ...
    rt_kprintf("\n");
	for (i = 0; i < pmadapter->num_in_scan_table; i++)
        rt_kprintf("Scan:(%02d) " MACSTR ", "
		       "RSSI[%03d], CHANNEL[%03d], SSID[%s]\n",
		       i,
		       MAC2STR(pmadapter->pscan_table[i].mac_address),
		       (t_s32) pmadapter->pscan_table[i].rssi,
               (t_s32) pmadapter->pscan_table[i].channel,
